    ...
    if embedding_engine == '':
        ...
        async def unavailable_embedding_function(query, prefix=None, user=None):
            raise ValueError(
                ...
                'Configure RAG_EMBEDDING_ENGINE as "openai" or "azure_openai".'
            )
...
class RerankCompressor(BaseDocumentCompressor):
    ...
    async def acompress_documents(
        ...
        if reranking:
            ...
            document_array = np.asarray(document_embedding, dtype=float)
            ...
            document_norm = document_array / (np.linalg.norm(document_array, axis=1, keepdims=True) + 1e-8)
            scores = (query_norm @ document_norm.T)[0].tolist()
