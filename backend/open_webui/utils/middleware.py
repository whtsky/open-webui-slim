...
async def process_chat_payload(request, form_data, user, metadata, model):
    ...
    # Chat Image Generation -> Chat Tools Function Calling -> Chat Files
...
async def get_event_emitter_and_caller(metadata):
    ...
    # websocket session (used by direct client-side tools).
...
async def background_tasks_handler(ctx):
    ...
    if 'chat_id' in metadata and not metadata.get('chat_id', '').startswith('local:'):
...
async def streaming_chat_response_handler(response, ctx):
    ...
    # event_caller is optional and only needed for direct client-side tools.
    if event_emitter:
        ...
        async def response_handler(response, events):
            def tag_output_handler(content_type, tags, output):
                """
                Detect special reasoning/solution tags in streaming content and
                create corresponding OR-aligned output items directly.
