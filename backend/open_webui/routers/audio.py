...
async def update_audio_config(request: Request, form_data: AudioConfigUpdateForm, user=Depends(get_admin_user)):
    if form_data.stt.ENGINE not in {'openai', 'deepgram', 'azure', 'mistral'}:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail='Open WebUI Slim requires an external STT provider.',
        )
    if form_data.tts.ENGINE not in {'openai', 'elevenlabs', 'azure', 'mistral'}:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail='Open WebUI Slim requires an external TTS provider.',
        )
...
async def transcription_handler(request, file_path, metadata, user=None):
    ...
    if engine not in {'openai', 'deepgram', 'azure', 'mistral'}:
        raise HTTPException(
            status_code=status.HTTP_501_NOT_IMPLEMENTED,
            detail='Configure an external STT engine before transcribing audio.',
        )
    if engine == 'openai':
