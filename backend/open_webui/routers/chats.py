...
async def generate_folder_chat_export_ndjson(folder_ids: list[str], user_id: str):
    skip = 0
    while True:
        chats = await Chats.get_chats_for_folder_export(
            folder_ids,
            user_id,
            skip=skip,
        ...
        if len(chats) < CHAT_EXPORT_BATCH_SIZE:
            break
...
async def get_chats_by_folder_id(
    ...
    return StreamingResponse(
        generate_folder_chat_export_ndjson(folder_ids, user.id),
...
async def generate_archived_chat_export_ndjson(user_id: str):
    skip = 0
    while True:
        chats = await Chats.get_archived_chats_by_user_id(
            user_id,
            skip=skip,
            limit=CHAT_EXPORT_BATCH_SIZE,
        )
...
async def get_user_archived_chats(user=Depends(get_verified_user)):
    return StreamingResponse(
        generate_archived_chat_export_ndjson(user.id),
        media_type='application/x-ndjson',
    )
