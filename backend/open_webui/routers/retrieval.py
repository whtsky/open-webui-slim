...
from open_webui.env import ENV, RAG_EMBEDDING_TIMEOUT
...
def get_ef(
    ...
    if embedding_model and engine == '':
        raise ValueError(
            'Local embedding models are unavailable in Open WebUI Slim. '
            'Configure the OpenAI or Azure OpenAI embedding engine.'
        )
    return None
...
def get_rf(
    ...
    if engine != 'external':
        raise ValueError(
            'Local reranking models are unavailable in Open WebUI Slim. Configure the external reranking engine.'
        )
    ...
    try:
        ...
        raise Exception(ERROR_MESSAGES.DEFAULT(e, 'Error loading reranking model')) from e
...
async def update_embedding_config(request: Request, form_data: EmbeddingModelUpdateForm, user=Depends(get_admin_user)):
    ...
    previous_ef = request.app.state.ef
    previous_embedding_function = request.app.state.EMBEDDING_FUNCTION
    await unload_embedding_model(request)
    try:
        if form_data.RAG_EMBEDDING_ENGINE not in {'openai', 'azure_openai'}:
            raise ValueError('Open WebUI Slim requires an external embedding engine: use "openai" or "azure_openai".')
        ...
        request.app.state.EMBEDDING_FUNCTION = get_embedding_function(
            ...
            (config.RAG_OPENAI_API_KEY if config.RAG_EMBEDDING_ENGINE == 'openai' else config.RAG_AZURE_OPENAI_API_KEY),
        ...
        request.app.state.ef = previous_ef
        request.app.state.EMBEDDING_FUNCTION = previous_embedding_function
...
def get_splitter_length_function(
    ...
    if config.TEXT_SPLITTER in {'token', 'token_transformers'}:
...
def save_docs_to_vector_db(
    ...
    if split:
        ...
        elif config.TEXT_SPLITTER in {'token', 'token_transformers'}:
