...
class ChatTable:
    ...
    async def get_message_by_id_and_message_id(self, id: str, message_id: str) -> dict | None:
        messages_map = await self.get_messages_map_by_chat_id(id)
    ...
    async def add_message_status_to_chat_by_id_and_message_id(
        ...
        if message_id in history.get('messages', {}):
            ...
            try:
                ...
                log.warning('Failed to write status history to chat_message table: %s', e)
    ...
    async def get_chat_by_id(
        ...
        """Fetch a chat by primary key without mutating the database."""
        try:
            async with get_async_db_context(db) as session:
                chat_item = await session.get(Chat, id)
                if chat_item is None:
                ...
                # Preserve the compatibility repair in the returned model, but
                # do not recursively sanitize or commit from a read path.
                self._repair_chat_current_id(chat_item.chat or {})
    ...
    async def get_chat_by_id_and_user_id(
        ...
        try:
            async with get_async_db_context(db) as session:
                ...
                self._repair_chat_current_id(chat.chat or {})
    ...
    async def get_archived_chats_by_user_id(
        ...
        async with get_async_db_context(db) as session:
            stmt = select(Chat).filter_by(user_id=user_id, archived=True).order_by(Chat.updated_at.desc(), Chat.id)
            if skip:
                stmt = stmt.offset(skip)
            if limit:
                stmt = stmt.limit(limit)
    ...
    async def get_chats_for_folder_export(
        self,
        folder_ids: list[str],
        user_id: str,
        skip: int = 0,
